...
async function run() {
  ...
  try {
    ...
    let result = await moov.representatives.list(
      credentials.connectedAccountID
    );
    ...
    result = await moov.representatives.create(
      credentials.connectedAccountID,
      newRep1
    );
    ...
    result = await moov.representatives.get(
      credentials.connectedAccountID,
      updateRep.representativeID
    );

    updateRep.email = "steve@classbooker.dev";

    // Update a specific representative
    result = await moov.representatives.update(
      credentials.connectedAccountID,
      updateRep.representativeID,
      updateRep
    );
    ...
    result = await moov.representatives.delete(
      credentials.connectedAccountID,
      updateRep.representativeID
    );
