...
export class BankAccounts {
  ...
  async link( accountID, bankAccount, plaidToken, mxAuthorizationCode ) {
    let payload = {};
    if (!accountID) {
      console.log("Please provide an accountID");
      return;
    }
    
    if (!bankAccount && !plaidToken && !mxAuthorizationCode) {
      console.log("Please provide a bankAccount, plaidToken, or mxAuthorizationCode.");
      return;
    }

    if (bankAccount) {
      if (!bankAccount.accountNumber) {
        console.log("Please provide a bank account number.");
        return;
      }
      if (!bankAccount.routingNumber) {
        console.log("Please provide a routingNumber.");
        return;
      }
      if (bankAccount.routingNumber.length !== 9) {
        console.log("Routing number must be 9 digits.");
        return;
      }
      if (!bankAccount.holderName) {
        console.log("Please provide a holderName.");
        return;
      }
      if (!bankAccount.holderType) {
        console.log("Please provide a holderType.");
        return;
      }
      payload = {
        account: bankAccount
